package com.fieldmotion.barcode;
...
public final class CameraManager {
	...
	public static int mDesiredWidth = 2000;
	public static int mDesiredHeight =2000;
	...
	public void clearPreview() {
		previewCallback.lastPreview=null;
		previewCallback.lastStrip=null;
	}
...
final class PreviewCallback implements Camera.PreviewCallback {
	...
	public byte[] lastPreview;
	public byte[] lastStrip;
		...
		synchronized (this)
       	{
			...
			imgs[0]=this.lastPreview;
			imgs[1]=this.lastStrip;
			return imgs;
		}
	}
	public void onPreviewFrame(byte[] data, Camera camera) {
		...
		try
		{
			...
			// { set up the 1px-high strip
			byte[] strip2;
			if (this.imageRotation!=0) {
				strip2=new byte[h];
				int yp=h*w-w/2;
				for (int i=0;i<h;++i, yp-=w) {
					strip2[i]=(byte)data[yp];
				}
			}
			else {
				strip2=new byte[w];
				int yp=w*(h/2);
				for (int i=0;i<w;++i) {
					strip2[i]=(byte)data[yp+i];
				}
			}
			// }
			int divider=2;
			while (w/divider>321 && h/divider>321) {
				divider*=2;
			}

			int[] rgbArray = new int[(w/divider) * (h/divider)];
			decodeYUV420SP(rgbArray, data, w, h, divider);
			w/=divider;
			h/=divider;
			...
			Bitmap preview=Bitmap.createBitmap(bitmap, 0, (h-w)/2, w, w);
			preview.compress(Bitmap.CompressFormat.JPEG, 85, previewStream);

			synchronized (this)
			{
				lastPreview=previewStream.toByteArray();
				lastStrip=strip2;
			}
	...
	static public void decodeYUV420SP(int[] rgb, byte[] yuv420sp, int width, int height, int divider) {
	    final int frameSize = width * height;

	    for (int j = 0, yp = 0, rgbidx=0; j < height; j+=divider, yp+=width*(divider-1)) {
	        int uvp = frameSize + (j >> 1) * width, u = 0, v = 0;
	        for (int i = 0, oddeven=0; i < width; i+=divider, yp+=divider, oddeven++) {
	            ...
	            rgb[rgbidx++] = 0xff000000 | ((r << 6) & 0xff0000) | ((g >> 2) & 0xff00) | ((b >> 10) & 0xff);
	        }
	    }
	}
}
