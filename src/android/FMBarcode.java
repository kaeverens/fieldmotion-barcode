...
public class FMBarcode extends CordovaPlugin {

    /** Common tag used for logging statements. */
    private static final String LOGTAG = "FMBarcode";
    ...
    private PluginResult getJpegImage(JSONArray inputs, CallbackContext callbackContext) {
        ...
        if (bArray != null)
        {
					...
					try {
						res.put("preview", Base64.encodeToString(bArray[0], Base64.NO_WRAP));
						res.put("strip", Base64.encodeToString(bArray[1], Base64.NO_WRAP));
    ...
    private boolean startCapture(){
        Log.w(LOGTAG, "startCapture");
				CameraManager.setDesiredPreviewSize(8000, 8000);
        try {
    ...
    private boolean stopCapture(){
        ...
        CameraManager.get().stopPreview();                
				CameraManager.get().clearPreview();
